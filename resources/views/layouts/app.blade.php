...
<body class="antialiased min-h-screen flex flex-col overflow-x-hidden font-sans">
    @php
        $siteName = \App\Models\Setting::valueOf('site_name', config('app.name', 'Sense of Jewels')) ?: config('app.name', 'Sense of Jewels');
        $siteLogo = \App\Models\Setting::valueOf('site_logo');
    @endphp
    ...
    <header class="sticky top-0 z-50 w-full">
        ...
        <div class="w-full bg-neutral text-neutral-content">
            <div class="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-4 text-[11px] tracking-wide">
                <span class="hidden sm:block">✦ &nbsp; {{ __('Sense of Jewels Studio · Seminyak, Bali') }}</span>
        ...
        <nav id="main-navbar" class="w-full bg-base-100 border-b border-base-200">
            ...
            <div class="lg:hidden flex items-center justify-between h-14 px-4">
                ...
                <a href="{{ url('/') }}" wire:navigate class="inline-flex h-10 items-center">
                    @if($siteLogo)
                        <img src="{{ $siteLogo }}" alt="{{ $siteName }}" class="w-auto object-contain" style="height: 26px;">
                    @else
            ...
            <div class="hidden lg:grid grid-cols-3 items-center h-[72px] max-w-7xl mx-auto px-6 lg:px-8">
                ...
                <nav class="flex items-center gap-8">
                    <a href="{{ url('/') }}" wire:navigate class="text-[11px] uppercase tracking-[0.18em] font-semibold transition-colors {{ request()->is('/') ? 'text-primary' : 'text-base-content/70 hover:text-primary' }}">{{ __('Home') }}</a>
                    <a href="{{ route('shop.index') }}" wire:navigate class="text-[11px] uppercase tracking-[0.18em] font-semibold transition-colors {{ request()->routeIs('shop.*') ? 'text-primary' : 'text-base-content/70 hover:text-primary' }}">{{ __('Shop') }}</a>
                ...
                <div class="flex justify-center">
                    <a href="{{ url('/') }}" wire:navigate class="{{ $siteLogo ? 'inline-flex h-14 items-center' : 'display-font text-[2.4rem] text-base-content hover:text-primary transition-colors leading-none tracking-wide' }}">
                        @if($siteLogo)
                            <img src="{{ $siteLogo }}" alt="{{ $siteName }}" class="w-auto object-contain" style="height: 38px;">
                        @else
                            {{ $siteName }}
                        @endif
                    </a>
                ...
                <div class="flex items-center justify-end gap-5">
                    ...
                    <!-- Account Icon -->
                    @auth
                    <a href="{{ route('profile.edit') }}" wire:navigate class="text-base-content/50 hover:text-base-content transition-colors">
                    ...
                    @else
                    <a href="{{ route('login') }}" wire:navigate class="text-base-content/50 hover:text-base-content transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    </a>
                    @endauth
                    @if($cartEnabled ?? true)
                    ...
                    <div class="flex items-center gap-1" x-data>
                        ...
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                            ...
                            <div x-show="open" x-transition class="absolute right-0 top-full mt-1 z-50 bg-base-100 border border-base-300 shadow-lg min-w-[90px]">
                                @foreach(['id' => __('Bahasa ID'), 'en' => 'English'] as $code => $label)
    ...
    <div class="bg-neutral text-neutral-content w-full">
        <footer class="footer p-10 lg:px-16 container mx-auto flex flex-col md:flex-row justify-between gap-8 py-16">
            ...
            <nav class="flex flex-col gap-3">
                ...
                <a href="#home" class="link link-hover text-neutral-content/80">{{ __('Home') }}</a>
                <a href="#collection" class="link link-hover text-neutral-content/80">{{ __('Collection') }}</a>
                <a href="#story" class="link link-hover text-neutral-content/80">{{ __('Brand Story') }}</a>
                <a href="#contact" class="link link-hover text-neutral-content/80">{{ __('Contact Us') }}</a>
            </nav>
    ...
    <div class="bg-neutral text-neutral-content px-10 border-t border-neutral-content/10">
        <div class="footer footer-center p-4 container mx-auto pb-8 pt-8">
            <aside>
                <p class="text-neutral-content/60">Copyright © {{ date('Y') }} - {{ __('All rights reserved by Sense of Jewels') }}</p>
            </aside>
        </div>
    </div>
    <script>
